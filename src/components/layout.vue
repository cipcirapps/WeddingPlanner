  ...
  <v-container fluid fill-height>
        ...
        <svg :width="Svg.w" :height="Svg.h">
          ...
          <g v-for="(shape,n,i) in Layout" :key="i">
            <rect
              ...
              :fill="shape.col"
            ></rect>
            <text :x="shape.x+'%'" :y="shape.y-0.5+'%'" class="txtLayout">{{shape.name}}</text>
          </g>
    ...
    <v-dialog persistent dark v-model="Stage.showRemain">
      <v-card>
        ...
        <v-card-text>
          <v-chip v-for="(om,i) in invRamasi" :key="i" @click="AddToLoc(om)" class="clickable">
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="error" flat @click="CancelAsignare()">Cancel</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
  ...
  methods: {
    ...
    populate() {
      ...
      this.InvArr.forEach(om => {
        if (om.Loc !== "") {
          vueOBJ.MeseData[parseInt(om.Masa) - 1].locuri[
            parseInt(om.Loc) - 1
          ].inv = om.Nume + " " + om.Prenume;
          vueOBJ.MeseData[parseInt(om.Masa) - 1].locuri[
            parseInt(om.Loc) - 1
          ].omID = om.id;
        }
    ...
    AddToLoc(om) {
      ...
      this.MeseData[this.Transfer.masa].locuri[
        this.Transfer.loc
      ].selected = false;
      //update om
